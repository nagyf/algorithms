...
import hu.nagyf.algorithms.graph.WeightedUndirectedGraph;

public class WeightedUndirectedGraphTest {

    @Test
    public void testWeightedEdge() {
        var graph = new WeightedUndirectedGraph<String, Integer>();
        graph.addEdge("A", "B", 10);

        Assert.assertEquals(10, (int)graph.weight("A", "B").get());
        Assert.assertEquals(10, (int)graph.weight("B", "A").get());
    }
